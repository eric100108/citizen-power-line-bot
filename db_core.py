...
def seed_base_data(conn):
    ...
    faq_rows = [
        ...
        ("site-planning", "開始前要先準備哪些資料？", "開始前先確認社區需求、可用屋頂、用電情境、補助資格與推動窗口，再進入場址評估。"),
        ...
        ("finance", "還不確定要不要做，先看哪些條件？", "可以先看 4 件事：1. 社區或場址是否有明確需求，2. 是否有可盤點的屋頂或合作空間，3. 是否有核心推動者，4. 是否願意投入溝通與行政時間。"),
        ...
        ("investment", "居民要怎麼參與投資？", "通常會先辦說明，確認投入金額、權利義務、回饋方式與退出規則，再依實際募集規則安排登記與後續參與。"),
        ...
        ("service-model", "第一次找真人協助前，要先準備什麼？", "建議先準備社區名稱、目前卡住的步驟、可用場址概況、是否想申請補助，以及你最在意的問題。"),
        ...
        ("project-progress", "南寮案現在進行到哪裡？", "目前可視為正式運轉階段，可從專案總覽與進度頁查看里程碑與進度紀錄。"),
        ("project-progress", "目前已經做出哪些成果？", "營運摘要可辨識已建置 27.3kW、11.83kW、23.205kW 三處屋頂場景，合計年發電量約 78,723 度、年營收約 393,615 元。"),
        ("service-model", "平台會用到哪些專案資料？", "會用到的資料包含計畫書、補助規則、案場資料與營運摘要。"),
    ]
    faq_rows.extend([
        ("site-planning", "屋頂坪數要怎麼換算成太陽光電容量？", "預設請使用可用面積估算：容量 kW = 可用面積 m2 / 5.0。若輸入的是總屋頂面積，會先乘上可用率；南寮案例模式則可切換為 4.91 m2/kW。"),
        ("site-planning", "太陽能板片數怎麼估？", "先用可用面積估容量，再用容量換算模組片數。預設以 410W/片估算，正式設計仍需依實際模組與現場排布確認。"),
        ("finance", "南寮的發電量試算用什麼基準？", "官方預設採 114 年澎湖平均 1,249 度/kW/年；南寮案例模式採 1,263 度/kW/年。長期試算可切換複利衰退或線性衰退，避免把不同 1% 衰退算法混在一起。"),
        ("finance", "南寮 2026 版收益分配怎麼看？", "2026 統整版採用股東紅利 50%、維運行政 35%、場址租金 10%、社區回饋 5% 作為財務模型基準。實際案場仍需依合約與治理規則確認。"),
    ...
    service_steps = [
        ...
        ("operation-feedback", "營運揭露與社區回饋", "長期營運", "community", "營運後持續更新發電、收益與回饋。", "固定揭露年發電量、收益、回饋用途與下一步擴點計畫。", 80, doc_company),
    ]

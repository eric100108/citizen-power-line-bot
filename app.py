...
def infer_user_intent(user_message):
    ...
    if any(term in normalized for term in ["真人協助", "找人協助", "聯絡真人", "真人", "人工協助"]):
        return "human_help"
    if "完整sop" in normalized or ("sop" in normalized and "完整" in normalized):
        return "full_sop"
    if any(term in normalized for term in ["怎麼開始", "如何開始", "從開始", "從哪開始", "從哪裡開始", "開始建立", "建立電廠", "啟動電廠"]):
        return "start_build"
    if "補助" in normalized:
        return "subsidy"
    if any(term in normalized for term in ["場址", "屋頂", "盤點"]):
        return "site"
    if any(term in normalized for term in ["到哪一步", "哪一步", "我的進度", "目前進度", "做到哪", "走到哪"]):
        return "progress"
    return None
